...
### 脸
### 头发
GF2 头发采用NPR渲染，分为**前发**和**后发**分开渲染，光照计算上二者基本相同。环境光（漫反射、高光）部分头发计算与衣服、丝袜计算一致，差异在直接光部分。
| 基础色贴图 | 高光贴图 | Ramp |
|------|------|------|
...
2. **直接光高光**：使用高光贴图存储前后头发的高光分布，并基于视角方向进行 UV 偏移，模拟头发反光随视角的动态变化。
```hlsl

```
   - 高光强度分配：阴影区域保留 10% 的亮度，避免高光完全消失
